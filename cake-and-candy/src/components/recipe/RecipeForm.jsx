  ...
  return (
    <div className="mb-4">
      ...
      <textarea
        ...
        className={`border p-2 mb-2 w-full resize-none ${errors.description ? 'border-red-500' : ''}`}
      ...
      {newRecipe.ingredients.map((ingredient, index) => (
        <div key={index} className="flex mb-2 items-center">
          <select
            ...
            className={`border p-2 mr-2 w-1/4 ${errors[`ingredient${index}name`] ? 'border-red-500' : ''}`}
          >
          ...
          <input
            ...
            className={`border p-2 w-1/4 ${errors[`ingredient${index}amount`] ? 'border-red-500' : ''}`}
          />
          <select
            value={ingredient.unit}
            onChange={(e) => handleIngredientChange(index, 'unit', e.target.value)}
            className={`border p-2 w-1/4 ${errors[`ingredient${index}unit`] ? 'border-red-500' : ''}`}
          >
          ...
          <button onClick={() => removeIngredient(index)} className="bg-red-500 text-white p-2 ml-2">Löschen</button>
        </div>
      ))}
      <button onClick={addIngredient} className="bg-teal-500 text-white p-2 mb-2">Zutat hinzufügen</button>
      <h3 className="text-lg font-semibold text-teal-400">Hilfsmittel</h3>
      <input
        type="text"
        placeholder="Hilfsmittel"
        value={newRecipe.tools.join(', ')}
        onChange={(e) => handleRecipeChange('tools', e.target.value.split(', '))}
      ...
      />
      <button onClick={saveRecipe} className="bg-teal-600 text-white p-2">{recipe.id ? 'Rezept aktualisieren' : 'Rezept speichern'}</button>
